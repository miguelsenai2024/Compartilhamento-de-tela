...
def ensure_connected_device():
    """Garante que existe um dispositivo ADB online via Wi-Fi. Se conectado via USB, configura Wi-Fi."""
    ...
    if devices:
        ...
        result = _run_adb(["shell", "ip", "route"])
        if "wlan0" in result.stdout or "wifi" in result.stdout.lower():
            print("✅ Celular conectado via Wi-Fi!")
            return True
        else:
            print("✅ Celular detectado via USB! Configurando para Wi-Fi...")
            # Enable tcpip
            result = _run_adb(["tcpip", "5555"])
            if result.returncode != 0:
                print("❌ Erro ao habilitar TCP/IP")
                return False
            time.sleep(1)
            # Get IP
            result = _run_adb(["shell", "ip", "route"])
            ...
            for line in lines:
                if "src" in line:
                    ...
                    break
            if not ip:
                print("❌ Erro ao obter IP do celular")
                return False
            print(f"IP do celular: {ip}")
            # Generate QR code with IP:5555
            qr = QrCode.encode_text(f"{ip}:5555")
            print("QR Code para conexão (escaneie com o celular se necessário):")
            _print_qr(qr)
            print("Agora desconecte o cabo USB e o script tentará conectar via Wi-Fi.")
            time.sleep(5)  # Wait for disconnect
            result = _run_adb(["connect", f"{ip}:5555"])
            if "connected" in result.stdout or "already connected" in result.stdout:
                print("✅ Conectado via Wi-Fi!")
                return True
            else:
                print("❌ Falha ao conectar via Wi-Fi. Tente novamente.")
                return False
    else:
        print("❌ Nenhum celular detectado.")
        print("Para configurar Wi-Fi debugging:")
        print(" 1. Conecte o cabo USB.")
        print(" 2. Ative a 'Depuração USB' no celular.")
        print(" 3. Desbloqueie a tela e aceite a permissão do PC.")
        print(" 4. Execute o script novamente para configurar Wi-Fi.")
        return False

def start_screen_mirror():
    """Abre a tela do celular no PC usando o scrcpy via Wi-Fi."""
    try:
        process = subprocess.Popen(
            [SCRCPY_PATH, '--tcpip', '--stay-awake'],
            creationflags=CREATE_NO_WINDOW
        )
        
        print("🚀 Tela espelhada com sucesso via Wi-Fi! (A tela do celular continua ligada)")
...
if __name__ == "__main__":
    print("=" * 50)
    print("  INICIALIZADOR DE TELA WIRELESS - WINDOWS 10  ")
    print("=" * 50)
